...
一册排盘的手记 —— 会说"这个我算不了"的命理 skill，因为它看不起那些什么都敢算的。
...
星野录（Star Fields）是一个 [Claude Code](https://claude.com/claude-code) skill：输入生辰，它会**精确排盘**、给出**有主见、不装神**的解读，并生成一张**东方古典书斋风格的可视化命盘**（独立 HTML，浏览器直接打开）。

它跟市面上的算命工具不一样的地方，只有一句话：

> **市面上九成的算命是一门恐惧生意——把话说得神神叨叨、无所不知，用一句"你命里有一劫"套住你，再卖你一个转运的方法。星野录站在它的对立面。**

- **技法上真专业** —— 排盘含真太阳时校正，四化、大限用数据说话（基于 [`iztro-py`](https://pypi.org/project/iztro-py/)）
...
| | |
|---|---|
| 🎯 **真太阳时排盘** | 按出生地经度校正钟表时间，避免卡时辰边界排错命宫 |
| 🗣 **诚实人格** | 三条铁律：区分能答/不能答、用错方法当场认、命盘是概率地图不是判决书 |
| 📅 **进阶模块** | 田宅宫四化算"哪年适合买房"、八宅命卦算个人方位、两人合盘——全部脚本化 |
| 🎨 **书斋视觉** | 米白纸感 + 朱砂红 + 赭金，印章篆刻、中英混排的古典命盘 |
| 🖐 **手相互证** | 可选：上传手相照片，与命盘交叉验证 |
| 🔧 **校准机制** | 解读后追问关键问题，把准确度从 ~70% 推向 85%+ |
...
星野录不预言死亡、不制造恐惧、不替代人生决策。它只说"这个格局倾向于……"，永远把行动权交还给你：
